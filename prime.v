...
Theorem pol_pow_0_mul_l {F : field} : ∀ s n i,
  i ≤ n
  → log_prod (ls (ls_of_pol (pol_pow 0))) (ls s) n i = f_zero.
Proof.
intros * Hin.
revert n Hin.
induction i; intros; [ easy | ].
rewrite log_prod_succ.
unfold log_prod_term.
replace (ls (ls_of_pol (pol_pow 0)) (n - i)) with f_zero. 2: {
  cbn.
