...
Definition b2n d := if ddec d then 1 else 0.
...
intros b Hb.
unfold b2n in Hb.
destruct (ddec b); [ discriminate Hb | assumption ].
Qed.
...
intros b Hb.
unfold b2n in Hb.
destruct (ddec b); [ assumption | discriminate Hb ].
Qed.
...
intros b.
unfold b2n.
destruct (ddec b); [ reflexivity | apply Nat.le_0_l ].
Qed.
...
unfold b2n; simpl.
destruct (ddec (x.[j-1])) as [Hxj|Hxj]; [ simpl | reflexivity ].
...
destruct s1 as [dj1| ].
 ...
 unfold b2n; simpl.
 destruct (ddec 0) as [H1|H1]; [ discr_digit H1 | reflexivity ].
Qed.
...
rewrite minus_plus.
destruct (ddec (x.[j])) as [Hx|Hx].
 destruct (ddec (y.[j])) as [Hy|Hy]; [ reflexivity | idtac ].
 rewrite Hxy, Hy in Hx; discr_digit Hx.

 destruct (ddec (y.[j])) as [Hy|Hy]; [ idtac | reflexivity ].
 rewrite Hxy, Hy in Hx; discr_digit Hx.
Qed.
...
destruct (ddec (x.[j-1])); [ idtac | apply Nat.le_0_l ].
destruct (ddec (y.[i-j])); [ reflexivity | apply Nat.le_0_l ].
Qed.
...
Proof.
intros x y n i Hin.
eapply Nat.le_trans; [ apply I_propag_carry_le | idtac ].
apply Nat.max_lub; [ idtac | reflexivity ].
...
Proof.
intros x y i.
assert (i - 1 ≤ S (S i)) as Hle1 by omega.
apply (I_propag_carry_mul_algo_le_1 x y) in Hle1.
assert (i - 1 ≤ S i) as Hle2 by omega.
apply (I_propag_carry_mul_algo_le_1 x y) in Hle2.
remember (I_propag_carry (I_mul_algo x y) (S (S i)) i) as v1 eqn:Hv1 .
remember (I_propag_carry (I_mul_algo x y) (S i) i) as v2 eqn:Hv2 .
symmetry in Hv1, Hv2.
destruct v1; [ clear Hle1 | idtac ].
 destruct v2; [ reflexivity | exfalso ].
 apply le_S_n, Nat.le_0_r in Hle2; subst v2.
 Focus 2.
 destruct v1; [ clear Hle1 | exfalso; omega ].
 destruct v2; [ clear Hle2; exfalso | idtac ].
  Focus 2.
  destruct v2; [ reflexivity | exfalso; omega ].

  Unfocus.
  remember (S i) as si; simpl in Hv1, Hv2; subst si.
  remember (I_propag_carry (I_mul_algo x y) (S i)) as u eqn:Hu .
  destruct i; simpl in Hv1.
   simpl in Hu.
   unfold propag_carry_once in Hv1; simpl in Hv1.
   remember (fst_not_1 u 1) as s1 eqn:Hs1 .
   apply fst_not_1_iff in Hs1; simpl in Hs1.
   destruct s1 as [di1| ].
    destruct Hs1 as (Hn1, Ht1).
    destruct (zerop (u (S di1))) as [H1| H1].
     rewrite Hv2 in Hv1; discriminate Hv1.

     clear Hv1.
     rewrite Hu in Hv2.
     unfold propag_carry_once in Hv2; simpl in Hv2.
     remember (fst_not_1 (I_mul_algo x y) 1) as s2 eqn:Hs2 .
     apply fst_not_1_iff in Hs2; simpl in Hs2.
     destruct s2 as [di2| ]; [ idtac | clear Hv2 ].
      destruct Hs2 as (Hn2, Ht2).
      destruct (zerop (I_mul_algo x y (S di2))) as [H2| H2].
       discriminate Hv2.

       clear Hv2.
       remember (u (S di1)) as m eqn:Hm .
       destruct m; [ revert H1; apply Nat.nlt_0_r | idtac ].
       destruct m; [ apply Ht1; reflexivity | clear Ht1 H1 ].
       symmetry in Hm.
       rewrite Hu in Hm; simpl in Hm.
       unfold propag_carry_once in Hm; simpl in Hm.
       remember (fst_not_1 (I_mul_algo x y) (S (S di1))) as s3 eqn:Hs3 .
       apply fst_not_1_iff in Hs3; simpl in Hs3.
       destruct s3 as [di3| ].
        destruct Hs3 as (Hn3, Ht3).
        destruct (zerop (I_mul_algo x y (S (S (di1 + di3))))) as [H1| H1].
         clear Ht3.
         destruct (le_dec (I_mul_algo x y (S di1)) 1) as [H3| H3].
          rewrite Hm in H3; revert H3; clear; intros; omega.
          ...
          destruct di2.
           ...
           rewrite Nat.add_0_r in H.
           unfold b2n in H, H1.
           destruct (ddec (x.[0])) as [Hx0|]; [ idtac | discriminate H ].
           destruct (ddec (y.[0])) as [Hy0|]; [ idtac | discriminate H ].
           clear H; simpl in H1.
...
bbb.
            ...
            rewrite Hx0, Hy0 in Hm; discriminate Hm.
              ...
              destruct (zerop (I_mul_algo x y (S (S di4)))) as [H3| H3].
               ...
               destruct (le_dec (I_mul_algo x y 1) 1) as [H4| H4].
                ...
                destruct dd.
                 ...
                 unfold I_mul_algo in Hm; simpl in Hm.
                 unfold summation in Hm; simpl in Hm.
                 rewrite Hx0, Hy0 in Hm; simpl in Hm.
                 rewrite Nat.mul_1_r, Nat.add_0_r in Hm.
                 rewrite Nat.add_0_r in Hm.
                 remember (x .[ 1]) as a.
                 remember (y .[ 1]) as b.
                 destruct a, b; discriminate Hm.

                 subst ss; simpl in H12.
                 rewrite Nat.sub_0_r in H31.
                 remember (S (S (S (S dd)))) as ss.
                 simpl in H13; subst ss.
                 apply Nat.eq_add_0 in H13.
                 ...
                 destruct di1.
                  ...
                  unfold I_mul_algo in H; simpl in H.
                  unfold summation in H; simpl in H.
                  rewrite Hx0, Hy0 in H; simpl in H.
      ...
      destruct s3 as [di3| ].
       ...
       destruct (zerop (I_mul_algo x y (S (S di3)))) as [H2| H2].
        ...
        destruct (le_dec (I_mul_algo x y 1) 1) as [H4| H4].
         ...
         unfold summation in H; simpl in H.
         rewrite Nat.add_0_r in H.
         remember (x .[ 0]) as a eqn:Hx .
         remember (y .[ 0]) as b eqn:Hy .
         symmetry in Hx, Hy.
         destruct a, b; try discriminate H; clear H.
         unfold I_mul_algo in H3; simpl in H3.
         unfold summation in H3; simpl in H3.
         rewrite Hx, Nat.sub_0_r in H3; simpl in H3.
         rewrite Nat.add_0_r in H3.
         destruct di2.
          simpl in H3.
          rewrite Hy in H3; discriminate H3.
          ...
          destruct di2.
           ...
           destruct di3.
            ...
            rewrite Hu in Hm; simpl in Hm.
            unfold propag_carry_once in Hm; simpl in Hm.
            remember (S (S (S di1))) as ss.
            remember (fst_not_1 (I_mul_algo x y) ss) as s2 eqn:Hs2 .
            apply fst_not_1_iff in Hs2; simpl in Hs2.
            destruct s2 as [di2| ].
             destruct Hs2 as (Hn2, Ht2).
             subst ss.
             remember (S (S (S (di1 + di2)))) as ss.
             destruct (zerop (I_mul_algo x y ss)) as [H1| H1].
              subst ss.
              clear Ht2.
              unfold I_mul_algo in H1; simpl in H1.
              unfold summation in H1; simpl in H1.
              rewrite Hx, H31 in H1; simpl in H1.
              ...
              destruct (le_dec (I_mul_algo x y (S (S di1))) 1) as [H4| H4].
               rewrite Hm in H4.
               apply le_S_n, Nat.le_0_r in H4.
               discriminate H4.

               apply Nat.add_sub_eq_nz in Hm;
                [ idtac | intros H; discriminate H ].
               symmetry in Hm; simpl in Hm.
               clear H4.
               unfold I_mul_algo in Hm; simpl in Hm.
               unfold summation in Hm; simpl in Hm.
