...
Theorem rm_add_i_vs_trunc_add_when_no_relay : ∀ a b a' b' i n c,
  ...
  → rm_add_i a b i = List.last (trunc_add_with_carry c a' b') false ⊕ negb c.
Proof.
intros a b a' b' i n c Hdi Ha' Hb'.
subst a' b'.
rewrite last_tr_add_with_carry.
 unfold rm_add_i; rewrite Hdi.
 do 2 rewrite last_trunc_from.
 rewrite last_carry_no_relay; auto.
 do 3 rewrite xorb_assoc; f_equal; f_equal.
 symmetry; rewrite xorb_comm.
 apply negb_xorb_diag.
...
Lemma last_carry_on_max : ∀ a b i d di dl,
...
destruct s as [di₁| ].
 simpl in Hd.
 remember Hs as H; clear HeqH.
 remember (List.fold_right max 0 dl) as n.
 eapply last_carry_through_relay with (c := true) (n := n - di₁) in H.
 rewrite <- Nat.sub_succ in H.
 rewrite <- Nat.add_succ_l in H.
 rewrite <- Hd in H.
 apply eq_S in Heqn.
 erewrite add_succ_sub_max in H; try eassumption.
 rewrite Hd, Nat.add_succ_r, <- Nat.add_succ_l, Hdi.
 rewrite orb_false_r; assumption.

 simpl in Hd; subst d.
 rewrite Nat.add_0_r.
 remember Hs as H; clear HeqH.
 eapply last_carry_no_relay in H.
...
bbb.
 remember (S (di + S n)) as m.
 unfold trunc_one.
 remember (rm_exp_opp (pred (m + i))) as c eqn:Hc .
 rewrite Heqm in Hc; simpl in Hc.
bbb.
 rewrite last_carry_through_relay.
