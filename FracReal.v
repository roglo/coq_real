...
Theorem A_ge_1_add_r_true_if2 {r : radix} : ∀ u i j k,
   A_ge_1 u (i + j) k = true
   → A_ge_1 u i (j + k) = true.
Proof.
intros *.
...
apply A_ge_1_true_iff in Hu.
apply A_ge_1_true_iff.
replace (i + (j + k) + 3) with (i + j + k + 3) by flia.
remember (rad * (i + j + k + 3)) as n eqn:Hn.
remember (n - (i + j) - 1) as s eqn:Hs.
move s before n.
assert (Hijn : i + j + 2 ≤ n - 1). {
...
replace (s + j - S (j + k)) with (s - S k) by flia Hs.
move Hu at bottom.
revert Hu.
apply Decidable.contrapositive; [ apply Nat.le_decidable | ].
intros Hu.
...
...
replace (rad ^ S (j + k) - 1) with
  (rad ^ S k - 1 + (rad ^ j - 1) * rad ^ S k) in Hu.
-rewrite Nat.mul_add_distr_r in Hu.
 apply Nat.add_lt_le_mono in Hu; [ easy | ].
 rewrite <- Nat.mul_assoc, Nat.mul_comm.
 rewrite <- Nat.pow_add_r.
 replace (S k + (s - S k)) with s.
 +apply Nat.mul_le_mono_pos_r.
