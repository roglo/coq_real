...
Theorem eq_all_numbers_to_digits_9_cond1 {r : radix} : ∀ u i n s j,
  (∀ k, u (i + k) ≤ 2 * (rad - 1))
  → s = n - i - 1
  → j < s
  → nA i n u mod rad ^ s < (rad ^ S j - 1) * rad ^ (s - S j)
...
Theorem glop {r : radix} : ∀ u n j j1 n1 s1,
  ...
  *rewrite summation_split with (e := n + j + 1); [ | flia Hj Hnn ].
  ...
  --rewrite <- summation_mul_distr_l.
    ...
    **rewrite H1, summation_empty; [ | apply Nat.lt_succ_diag_r ].
      ...
      assert (Hjj : S j1 ≤ j). {
        rewrite Hn1 in H1.
        destruct rad; [ easy | simpl in H1; flia H1 ].
      }
      do 2 rewrite Nat.mul_sub_distr_r, Nat.mul_1_l.
      do 2 rewrite <- Nat.pow_add_r.
      replace (S j1 + (s1 - S j1)) with s1 by flia Hs1 H1 Hjj.
