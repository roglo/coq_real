...
Theorem carry_succ_lemma {r : radix} : ∀ m u i j a n,
  0 < m ≤ rad
  → i ≤ j + 1
  → (∀ k, u (i + k) ≤ m * (rad - 1))
  → a = A j n u
...
Proof.
intros * (Hmz, Hmr) Hij Hur Ha H3.
rewrite (Q.frac_small (_ * _)%Q) in H3. 2: {
  rewrite Ha.
  apply (A_mul_inv_rad_interv m _ i); [ easy | easy | easy ].
}
...
destruct (zerop m) as [Hmz| Hmz]. {
  ...
  rewrite all_0_summation_0. 2: {
    ...
    specialize (Hur (j - i)) as H1.
    replace (i + (j - i)) with j in H1 by flia Hj.
    now apply Nat.le_0_r in H1.
  }
  rewrite all_0_summation_0; [ now rewrite Nat.div_0_l | ].
  intros j Hj.
  replace (u j) with 0; [ easy | ].
  specialize (Hur (j - i)) as H1.
  replace (i + (j - i)) with j in H1 by flia Hj.
...
destruct (LPO_fst (fA_ge_1_ε u i)) as [H1| H1]. {
  ...
  destruct (LPO_fst (fA_ge_1_ε u (i + 1))) as [H2| H2]. {
    ...
    destruct
      ...
      as [H3| H3]. {
      rewrite Nat.add_0_r.
      apply (carry_succ_lemma m _ _ (i + 1) _ (min_n i 1)); try easy; flia.
    }
    specialize (Nat.div_mod (u (i + 1)) rad radix_ne_0) as H5.
    symmetry; rewrite H5 at 1.
    rewrite Nat.mul_comm, <- Nat.add_assoc, Nat.add_comm.
...
induction k. {
  ...
  destruct (Nat.eq_dec m 1) as [Hm1| Hm1]. {
    ...
    unfold P, d2n, prop_carr, dig in H1.
    specialize (carry_upper_bound_for_adds 1 u i (Nat.neq_succ_0 _)) as H2.
    assert (H : ∀ k, u (i + k + 1) ≤ 1 * (rad - 1)). {
      now intros; rewrite <- Nat.add_assoc, Nat.mul_1_l.
    }
    specialize (H2 H 2); clear H.
    apply Nat.lt_1_r in H2; rewrite H2, Nat.add_0_r in H1.
    rewrite Nat.mod_small in H1; [ easy | ].
    specialize (Hur 2) as H3.
    flia Hr H3.
  }
  move Hm1 before Hmz.
