...
-clear H1.
 rewrite H2 in Hum.
 assert (H1 : m mod rad = 1). {
   rewrite <- Nat.mul_mod_idemp_l in Hum; [ | easy ].
   rewrite <- Nat_mod_add_same_r in Hum; [ | easy ].
   rewrite Nat.mul_sub_distr_l, Nat.mul_1_r in Hum.
   rewrite <- Nat.add_sub_swap in Hum. 2: {
     rewrite Nat.mul_comm.
     destruct rad; [ easy | cbn; flia ].
   }
   rewrite <- Nat.add_sub_assoc in Hum. 2: {
     now apply Nat.lt_le_incl, Nat.mod_upper_bound.
   }
   rewrite Nat.add_comm in Hum.
   rewrite Nat.mod_add in Hum; [ | easy ].
   destruct (zerop (m mod rad)) as [H1| H1].
   -rewrite H1, Nat.sub_0_r in Hum.
    rewrite Nat.mod_same in Hum; [ flia Hum Hr | easy ].
   -rewrite Nat.mod_small in Hum; [ | flia H1 Hr ].
    flia Hum Hr.
 }
 specialize (P_999_start u (i + j + 1) m) as H3.
 ...
 +clear H3.
  destruct (lt_dec m rad) as [H3| H3].
  *rewrite Nat.mod_small in H1; [ | easy ].
   subst m; rewrite Nat.mul_1_l in H4.
   exists 1; easy.
  *apply Nat.nlt_ge in H3.
(* mmmm... ça chie, ça ; peut-être le théorème lui-même *)
...
   specialize (Nat.div_mod m rad radix_ne_0) as H5.
   rewrite H1 in H5.
...
  specialize (Nat.div_mod m rad radix_ne_0) as H3.
  rewrite H3, H1 in H4.
  rewrite Nat.mul_add_distr_r, Nat.mul_1_l in H4.
  rewrite Nat.mul_sub_distr_l, Nat.mul_1_r in H4.
...
  replace m with (1 + (m - 1)) in H4. 2: {
...
  }
  rewrite Nat.mul_sub_distr_l, Nat.mul_1_r in H4.
  rewrite Nat.mul_add_distr_r, Nat.mul_1_l in H4.
...
-rewrite Hnv, Hnuv at 1.
  ...
  *destruct (NQlt_le_dec (A i nuv u + A i nuv v) 1) as [H5| H5]; [ | easy ].
  ...
  --subst kup; rewrite <- Hnv in Hnup; subst nup.
   ...
   ++exfalso; apply NQnle_gt in H5; apply H5; clear H5.
     ...
     destruct p. {
       rewrite Hnuv in Hp; unfold min_n in Hp.
       destruct rad; [ easy | cbn in Hp; flia Hp ].
     }
     cbn; replace 2 with (2 * 1) by easy.
     apply Nat.mul_le_mono; [ easy | ].
     now apply Nat_pow_ge_1.
   ++destruct H4 as (j & Hbef & Hwhi & Haft).
     destruct (lt_dec nuv (i + j + 2)) as [H4| H4].
    **rewrite A_all_9; [ | easy ].
      rewrite A_all_9; [ | intros k Hk; apply Hbef; flia H4 Hk ].
      eapply NQle_trans; [ | apply NQle_add_r, B_ge_0 ].
      apply NQsub_le_mono; [ apply NQle_refl | ].
      apply NQle_pair; [ pauto | pauto | ].
      rewrite Nat.mul_1_r, Nat.mul_1_l.
