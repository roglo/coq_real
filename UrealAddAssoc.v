...
destruct j. {
  ...
  destruct c3. {
    ...
    rewrite <- (Nat.add_0_r (i + 2)) in Hpu2 at 2 by easy.
    ...
    rewrite min_n_add, Nat.mul_1_r, Hr2 in Hc3.
...
apply rad_2_sum_3_all_9_3r2_3r2; [ easy | now rewrite Hr2 | easy | ].
Abort. (*
...
    apply Q.eq_intg_0 in Hc3; [ | easy ].
    now apply Q.nle_gt in Hc3.
...
Theorem rad_2_glop {r : radix} : ∀ m j k u v i n,
  ...
  → (u ⊕ v) (i + 1) = 0 ∨ (u ⊕ v) (i + 1) = 2
  → (∀ k, k < m → (u ⊕ v) (i + k + 2) = 2)
  → ((carry v (i + 1) mod 2) // 2 + A (i + 1) n (u ⊕ P v) * 1 // 2 < 1)%Q.
Proof.
intros * Hr2 Hin Hu Hv Hjj Hj Hjk Hk Hauv Huv1 Huv2.
assert (Huvl3 : ∀ k, (u ⊕ v) (i + k) ≤ 3). {
  intros p.
  unfold "⊕"; replace 3 with (1 + 2) by easy.
  apply Nat.add_le_mono; [ apply Hu | apply Hv ].
}
move Huvl3 before Hauv.
...
destruct Huv2' as [Huv2'| Huv2']. {
  specialize (rad_2_sum_3_all_9_02_1_333 _ i Hr2 Huvl3 Hauv Huv1) as Huvn.
  specialize (Huvn Huv2').
  ...
  destruct Huv2' as [Huv2'| Huv2']. {
    rewrite A_all_18. 2: {
      ...
      destruct p. {
        ...
        symmetry; unfold carry.
        rewrite A_split_first; [ | min_n_ge ].
        replace (S (i + 2)) with (i + 0 + 3) by flia.
        rewrite Hvn, Hr2.
        rewrite (Q.intg_add_nat_l 1); [ | now apply Q.le_0_mul_r ].
      ...
      replace (carry v (i + p + 3)) with 1; [ easy | ].
      symmetry; unfold carry.
      rewrite A_split_first; [ | min_n_ge ].
      replace (S (i + p + 3)) with (i + (p + 1) + 3) by flia.
      rewrite Hvn, Hr2.
      rewrite (Q.intg_add_nat_l 1); [ | now apply Q.le_0_mul_r ].
      symmetry; replace 1 with (1 + 0) at 1 by easy; symmetry; f_equal.
      apply Q.intg_small.
    ...
    replace (carry v (i + 1)) with 0. 2: {
      ...
      rewrite A_split_first; [ | min_n_ge ].
      replace (S (i + 1)) with (i + 2) by flia.
      rewrite (proj2 Huv2'), Hr2, Q.add_0_l.
      apply Q.intg_small.
      split; [ now apply Q.le_0_mul_r | ].
      apply rad_2_sum_2_half_A_lt_1; [ easy | ].
      now intros; rewrite <- Nat.add_assoc.
    ...
    rewrite Q.add_0_l, Q.mul_sub_distr_r.
  ...
  replace (P v (i + 2)) with 0. 2: {
    symmetry; cbn; rewrite (proj2 Huv2'), Hr2.
    replace (carry v (i + 2)) with 1; [ easy | ].
    symmetry; unfold carry.
    rewrite A_split_first; [ | min_n_ge ].
    replace (S (i + 2)) with (i + 0 + 3) by flia.
...
destruct Huv2' as [Huv2'| Huv2']. {
  specialize (rad_2_sum_3_all_9_02_222_13 m (u ⊕ v) i Hr2) as Huv2''.
  assert (H : ∀ k, (u ⊕ v) (i + k + 1) ≤ 3). {
    now intros; rewrite <- Nat.add_assoc.
  }
...
...
      destruct (LPO_fst (λ k, Nat.eqb (v (i + p + k + 3)) 1)) as [H1| H1]. {
        ...
        rewrite (A_9_8_all_18 p); cycle 1. {
          ...
          clear H1.
          rewrite <- (carry_nth_carry 2 (p - q - 1)); cycle 1. {
            rewrite Hr2.
            replace 2 with (2 ^ 1) at 1 by easy.
          ...
          replace p with (q + s + 1) in Hv1, Hp, Hjp by flia Hs Hq.
          clear Hs Hq Hcp2.
          revert q Hjp Hp Hv1.
          induction s; intros. {
            cbn.
            ...
            rewrite Hp, Hr2.
            replace (i + (q + 1) + 2) with (i + q + 3) by flia.
            specialize (carry_upper_bound_for_adds 2 v i) as H1.
            assert (H : 2 ≠ 0) by easy.
            specialize (H1 H); clear H.
            assert (H : ∀ k, v (i + k + 1) ≤ 2 * (rad - 1)). {
              now intros s; rewrite <- Nat.add_assoc, Hr2.
            }
          ...
          replace (i + (q + S s + 1)) with (i + (S q + s + 1)) in Hp by flia.
          replace (i + (q + S s + 1)) with (i + (S q + s + 1)) in Hv1 by flia.
          replace (q + S s) with (S q + s) in Hjp by flia.
          specialize (IHs (S q) Hjp Hp Hv1) as H1.
          cbn.
          replace (i + S q + 2) with (i + q + 2 + 1) in H1 by flia.
          rewrite H1.
          replace (i + q + 2 + 1) with (i + S q + 2) by flia.
          rewrite Hjp, Hr2; [ easy | flia ].
        } {
          replace (i + 1 + p + 1) with (i + p + 2) by flia.
          unfold "⊕"; rewrite Hr2.
...
destruct Huv2 as [Huv2| Huv2]. {
  ...
  destruct Huv2 as [Huv2| Huv2]. {
    ...
    rewrite Nat.mul_add_distr_r, Nat.mul_1_l; f_equal. {
      ...
      rewrite Hr2.
      rewrite (Q.intg_less_small 1). {
        destruct p; [ now rewrite Nat.add_0_r, (proj2 Huv2) | ].
        replace (i + S p + 2) with (i + p + 3) by flia.
        now rewrite Hvn.
      }
      rewrite Q.pair_diag; [ | easy ].
...
destruct Huv2 as [Huv2| Huv2]. {
  ...
  destruct Huv2 as [Huv2| Huv2]. {
    ...
    destruct Huv3 as [Huv3| Huv3]. {
      ...
      assert (Hun : ∀ k, u (i + k + 4) = 1). {
        intros p.
        specialize (Huvn p) as (H1, _).
        unfold "⊕" in H1.
        specialize (Hu (p + 4)) as H2; rewrite Nat.add_assoc in H2.
...
*)
      ...
      rewrite (A_9_8_all_18 0); cycle 1; [ now intros p Hp | | | ]. {
        replace (i + 2 + 0 + 1) with (i + 3) by flia.
        rewrite Hr2.
        unfold "⊕"; rewrite (proj1 Huv3), Nat.add_0_l.
        unfold P, d2n, prop_carr, dig.
        ...
        intros p.
        replace (i + 2 + 0 + p + 2) with (i + p + 4) by flia.
        unfold "⊕"; rewrite Hr2, Hun, Nat.add_comm.
...
...
        rewrite Q.mul_add_distr_r, Q.mul_1_l, Q.add_assoc.
          now rewrite <- Q.mul_assoc; apply Q.le_0_mul_r.
        }
        split; [ now rewrite <- Q.mul_assoc; apply Q.le_0_mul_r | ].
        apply (Q.lt_le_trans _ (1 * 1 // 2)%Q). 2: {
          rewrite Q.mul_1_l.
          apply (Q.le_pair _ _ 1 1); [ easy | easy | flia ].
        }
        apply Q.mul_lt_mono_pos_r; [ easy | ].
        apply rad_2_sum_2_half_A_lt_1; [ easy | ].
        now intros; do 2 rewrite <- Nat.add_assoc.
      } {
        replace (i + 1 + 1 + 1) with (i + 3) by flia.
        unfold "⊕"; rewrite (proj1 Huv3), Nat.add_0_l, Hr2; cbn.
        rewrite (proj2 Huv3), Hr2.
        replace (carry v (i + 3)) with 1; [ easy | ].
        symmetry; unfold carry.
        rewrite A_split_first; [ | min_n_ge ].
        ...
        intros p; replace (i + 1 + 1 + p + 2) with (i + p + 4) by flia.
        unfold "⊕"; rewrite Hr2; cbn; rewrite Hr2.
        rewrite Hun, Hvn; replace 2 with (1 + 1) at 4 by easy; f_equal.
        rewrite Nat_mod_add_same_l; [ | easy ].
        replace (carry v (i + p + 4)) with 1; [ easy | ].
        symmetry; unfold carry.
        rewrite A_all_18. 2: {
          intros q; rewrite Hr2; cbn.
          now replace (i + p + 4 + q + 1) with (i + (p + q + 1) + 4) by flia.
        }
        apply (Q.intg_less_small 1).
        rewrite Q.pair_diag; [ | easy ].
        split. {
          apply Q.le_add_le_sub_l.
          replace 2%Q with (1 + 1)%Q by easy.
          ...
          apply (Q.le_pair _ _ 1 1); [ pauto | easy | ].
          do 2 rewrite Nat.mul_1_r; rewrite Hr2.
          replace 2 with (2 ^ 1) at 1 by easy.
          apply Nat.pow_le_mono_r; [ easy | min_n_ge ].
        }
        apply Q.sub_lt, Q.lt_0_pair; pauto.
...
...
  ...
  destruct Huv2 as [Huv2| Huv2]. {
    ...
    destruct Huv3 as [Huv3| Huv3]. {
      ...
      (* if true, perhaps case with j=0 could be useless and deleted *)
      (*     u 0 1 0 1 1 1 1 ...
             v 0 1 1 2 2 2 2 ...
          u⊕v 0 2 1 3 3 3 3 ...
            Pv 1 0 0 1 1 1 1 ...
      ...
      rewrite (A_9_8_all_18 2); cycle 1. {
        ...
        destruct p; [ clear Hp | flia Hp ].
        replace (i + 1 + 1) with (i + 2) by flia.
        rewrite (proj1 Huv2), (proj2 Huv2), Hr2.
        symmetry; replace 1 with (1 + 0) at 1 by easy; symmetry; f_equal.
        replace (carry v (i + 2)) with 1; [ easy | ].
        symmetry.
        unfold carry.
        ...
        apply Q.intg_small.
        split; [ now apply Q.le_0_mul_r | ].
        apply (Q.mul_lt_mono_pos_r 4); [ easy | ].
        rewrite <- Q.mul_assoc.
        rewrite (Q.mul_pair_den_num _ 4 1); [ | easy ].
        rewrite Q.mul_1_r, Q.mul_1_l.
        apply (Q.lt_le_trans _ 2). 2: {
          apply (Q.le_pair_mono_r 2 4 1); pauto.
        }
        eapply Q.le_lt_trans. {
          apply (A_upper_bound_for_adds 2).
          now intros; rewrite Hr2; do 2 rewrite <- Nat.add_assoc.
        ...
        unfold "⊕", P, d2n, prop_carr, dig.
        rewrite (proj1 Huv3), (proj2 Huv3), Nat.add_0_l, Hr2, Nat.sub_diag.
        replace (carry v (i + 3)) with 1; [ easy | ].
        symmetry.
        unfold carry.
        rewrite A_split_first; [ | min_n_ge ].
        ...
        intros p.
        unfold "⊕"; rewrite Hr2; cbn.
        replace (i + 2 + p + 2) with (i + p + 4) by flia.
        rewrite Hvn, Hr2, Nat_mod_add_same_l; [ | easy ].
        specialize (Huvn p) as (Huv, Hcuv).
        unfold "⊕" in Huv; rewrite Hvn in Huv.
        replace (u (i + p + 4)) with 1 by flia Huv.
        symmetry; replace 2 with (1 + 1) at 1 by easy; symmetry; f_equal.
        rewrite Nat.mod_small. 2: {
          specialize (carry_upper_bound_for_add v (i + p + 4)) as H1.
          assert (H : ∀ k, v (i + p + 4 + k + 1) ≤ 2 * (rad - 1)). {
      ...
      rewrite Q.frac_small. 2: {
        split. {
          ...
          do 2 rewrite Nat.mul_1_r.
          apply (le_trans _ 2). {
            destruct (le_dec (i + 2 + 1) (min_n (i + k) - 1)); [ easy | pauto ].
          }
          unfold min_n; rewrite Hr2; cbn.
          replace 2 with (2 ^ 1) at 1 by easy.
          apply Nat.pow_le_mono_r; [ easy | flia ].
        }
        apply Q.sub_lt, Q.lt_0_pair.
        destruct (le_dec (i + 2 + 1) (min_n (i + k) - 1)); pauto.
      }
