...
Theorem frac_ge_if_all_fA_ge_1_ε_le_rad {r : radix} : ∀ u i,
  (∀ k, u (i + k) ≤ 2 * (rad - 1))
  → (∀ k, fA_ge_1_ε u i k = true)
  → ∀ k l, l ≤ rad
...
(* remonter avant ce théorème *)
...
split.
-intros H1 k l.
 destruct (le_dec l rad) as [H2| H2].
 +now apply frac_ge_if_all_fA_ge_1_ε_le_rad.
 +apply Nat.nle_gt in H2.
  specialize (Nat.div_mod l rad) as H3.
  assert (H : rad ≠ 0) by easy.
  specialize (H3 H); clear H.
  specialize (frac_ge_if_all_fA_ge_1_ε_le_rad u i Hur H1) as H4.
...
rewrite NQintg_add_frac in H3.
rewrite (NQfrac_small (B _ _ _ _)) in H3. 2: {
  split; [ apply B_ge_0 | ].
  apply B_lt_1; [ easy | ].
  rewrite Hn; unfold min_n.
  destruct rad; [ easy | cbn; flia ].
...
assert (NQintg (A i nv v) = NQintg (A i nuv v)). {
  ...
  -destruct (LPO_fst (fA_ge_1_ε (u ⊕ v) i)) as [H4| H4]; [ easy | ].
   ...
   rewrite (NQfrac_small (B _ _ _ _)) in H4. 2: {
     split; [ apply B_ge_0 | ].
     rewrite Hn.
     eapply NQlt_trans.
     -apply B_upper_bound_for_add.
