...
Theorem glop {r : radix} : ∀ u i,
  ¬ ∀ k : nat, d2n (prop_carr u) (i + k + 1) = rad - 1.
Proof.
intros * Hk.
specialize (Hk 0) as H1; rewrite Nat.add_0_r in H1.
unfold d2n, prop_carr in H1; simpl in H1.
unfold nat_prop_carr in H1.
destruct (LPO_fst (A_ge_1 u (i + 1))) as [H2| H2].
-replace (i + 1 + 3) with (i + 4) in H1 by flia.
 remember (rad * (i + 4)) as n1 eqn:Hn1.
 replace (n1 - (i + 1) - 1) with (n1 - i - 2) in H1 by flia.
 remember (n1 - i - 2) as s1 eqn:Hs1.
 move s1 before n1.
 assert
   (H4 : ∀ k (n := rad * (i + k + 4)) (s := n - i - 2),
   nA (i + 1) n u mod rad ^ s ≥ (rad ^ S k - 1) * rad ^ (s - S k)). {
   intros k.
   specialize (H2 k).
   apply A_ge_1_true_iff in H2.
   replace (i + 1 + k + 3) with (i + k + 4) in H2 by flia.
   remember (rad * (i + k + 4)) as n eqn:Hn.
   now replace (n - (i + 1) - 1) with (n - i - 2) in H2 by flia.
 }
 specialize (H4 0) as H5.
